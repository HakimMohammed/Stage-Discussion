...
export default function PrivateMessageChamp(props) {

    const idDiscussion = props.discussion.idDiscussionPrive;
    ...
    useEffect(() => {
        ...
        async function fetchData(id) {
            await axios.get(`http://127.0.0.1:8000/api/privates-messages/${id}`)
                .then((res) => {
                    console.log(res.data)
        ...
        // Broadcasting -----------------------------------------------------------------------------
        const channel = window.Echo.private(`private_discussion_${idDiscussion}`);

        channel.listen('sendPrivateMessageEvent', (data) => {
    ...
    const handleSubmit = async (event) => {
        ...
        await axios.post('http://127.0.0.1:8000/api/private-message/send', { loggedIn, loggedType, idDiscussion, contenu , msgType })
            .then((res) => {
        ...
        if (msgType == 'audio') {
            ...
            await axios.post('http://127.0.0.1:8000/api/private-message/send', formData)
                .then((res) => {
                    console.log(res.data);
                })
                .catch((err) => {
                ...
                <div class="d-flex justify-content-between">
                    <div class="d-flex align-items-center">
                        ...
                        <figure class="mb-0 me-2">
                            {loggedType == 'Professeur' && <DiscussionPicture name={`${props.discussion.tuteur.nom} ${props.discussion.tuteur.prenom}`} />}
                            {loggedType == 'Tuteur' && <DiscussionPicture name={`${props.discussion.professeur.nom} ${props.discussion.professeur.prenom}`} />}
                        </figure>
                        <div>
                            {loggedType == 'Professeur' && <p>{`${props.discussion.tuteur.nom} ${props.discussion.tuteur.prenom}`}</p>}
                            {loggedType == 'Tuteur' && <p>{`${props.discussion.professeur.nom} ${props.discussion.professeur.prenom}`} </p>}
                        </div>
                    </div>
